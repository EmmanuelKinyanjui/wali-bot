...
async function loadDevice () {
  ...
  if (config.device && !config.device.includes(' ')) {
    if (/^[a-f0-9]{24}$/i.test(config.device) === false) {
      return exit('Invalid WhatsApp device ID: must be 24 characers hexadecimal value. Get the device ID here: https://web.whatsawa.com/number')
    }
...
async function main () {
  ...
  if (!config.apiKey || config.apiKey.length < 60) {
    return exit('Please sign up in WhatSawa and obtain your API key here:\nhttps://web.whatsawa.com/apikeys')
  }
  ...
  if (!device) {
    return exit('No active WhatsApp numbers in your account. Please connect a WhatsApp number in your WhatSawa account:\nhttps://web.whatsawa.com/create')
  }
  if (device.session.status !== 'online') {
    return exit(`WhatsApp number (${device.alias}) is not online. Please make sure the WhatsApp number in your WhatSawa account is properly connected:\nhttps://web.whatsawa.com/${device.id}/scan`)
  }
